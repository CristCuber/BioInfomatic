...
public class GlobalSequence {
    ...
    public static void main(String[] args) {
        ...
        printSol(asw1, asw2, stl, sol, select, sOne, sTwo); //print solution
        System.out.println("score: " + table[stl][sol]);
    }
    
    static void printSol(String sol1, String sol2, int x, int y, Path[][] p, String seq1, String seq2) {
        if(p[x][y].canTopleft()) {
            String tmp1 = Character.toString(seq1.charAt(y - 1)).concat(sol1);
            String tmp2 = Character.toString(seq2.charAt(x - 1)).concat(sol2);
            printSol(tmp1, tmp2, x - 1, y - 1, p, seq1, seq2);
        }
        
        if(p[x][y].canTop()) {
            String tmp1 = Character.toString('-').concat(sol1);
            String tmp2 = Character.toString(seq2.charAt(x - 1)).concat(sol2);
            printSol(tmp1, tmp2, x - 1, y, p, seq1, seq2);
        }
        
        if(p[x][y].canLeft()) {
            String tmp1 = Character.toString(seq1.charAt(y - 1)).concat(sol1);
            String tmp2 = Character.toString('-').concat(sol2);
            printSol(tmp1, tmp2, x, y - 1, p, seq1, seq2);
        }
